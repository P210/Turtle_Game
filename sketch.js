...
if(turtle.isTouching(bagGroup||cocacolaGroup)){ 
...
gameState = "end"
    ...
    if(gameState=== "end"){
      ...
      cocacolaGroup.setVelocityXEach(0);
      bg.velocityX = 0;
      score=0;
      play.visible = true;
      spawnMessage1();
...
function isTouching(){
  ...
  function ifTouching(){
    if(turtle.x-cocacola.x<turtle.height/2+cocacola.height/2
      && cocacola.x-turtle.x<turtle.height/2+cocacola.height/2
      && cocacola.y-turtle.y<turtle.width/2+ cocacola.width/2
      && turtle.y- cocacola.y<turtle.width/2 + cocacola.width/2){
     return true;
    }
    else {
      return false;
